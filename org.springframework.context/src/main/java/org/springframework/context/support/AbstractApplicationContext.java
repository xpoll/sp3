...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public AbstractApplicationContext(ApplicationContext parent) {
		this.parent = parent;
		System.err.println("** AbstractApplicationContext#构造方法--");
		this.resourcePatternResolver = getResourcePatternResolver();
		System.err.println("** AbstractRefreshableApplicationContext#构造方法.getResourcePatternResolver--return new PathMatchingResourcePatternResolver(this)");
		this.environment = createEnvironment();
        System.err.println("** AbstractRefreshableApplicationContext#构造方法.createEnvironment--return new StandardEnvironment()");
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
		    System.err.println("** AbstractApplicationContext#.refresh()--准备刷新上下文。Prepare this context for refreshing. ");
			prepareRefresh();

            System.err.println("** AbstractApplicationContext#.refresh()--告诉子类刷新内部bean工厂。Tell the subclass to refresh the internal bean factory. ");
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

            System.err.println("** AbstractApplicationContext#.refresh()--准备在这个上下文中使用的bean工厂。Prepare the bean factory for use in this context. ");
			prepareBeanFactory(beanFactory);

			try {
	            System.err.println("** AbstractApplicationContext#.refresh()--允许在上下文子类中对bean工厂进行后处理。Allows post-processing of the bean factory in context subclasses. ");
				postProcessBeanFactory(beanFactory);

	            System.err.println("** AbstractApplicationContext#.refresh()--在上下文中调用在Bean中注册的工厂处理器。Invoke factory processors registered as beans in the context. ");
				invokeBeanFactoryPostProcessors(beanFactory);

	            System.err.println("** AbstractApplicationContext#.refresh()--注册bean处理器拦截bean创建。Register bean processors that intercept bean creation. ");
				registerBeanPostProcessors(beanFactory);

	            System.err.println("** AbstractApplicationContext#.refresh()--初始化此上下文的消息源。Initialize message source for this context. ");
				initMessageSource();

	            System.err.println("** AbstractApplicationContext#.refresh()--为此上下文初始化事件多重数据。Initialize event multicaster for this context. ");
				initApplicationEventMulticaster();

	            System.err.println("** AbstractApplicationContext#.refresh()--在特定的上下文子类中初始化其他特殊的bean。Initialize other special beans in specific context subclasses. ");
				onRefresh();

	            System.err.println("** AbstractApplicationContext#.refresh()--检查监听器bean并注册它们。Check for listener beans and register them. ");
				registerListeners();

	            System.err.println("** AbstractApplicationContext#.refresh()--实例化所有剩下的（非懒惰init）单例。Instantiate all remaining (non-lazy-init) singletons. ");
				finishBeanFactoryInitialization(beanFactory);

	            System.err.println("** AbstractApplicationContext#.refresh()--最后一步：发布相应的事件。Last step: publish corresponding event. ");
				finishRefresh();
			}

			catch (BeansException ex) {
	            System.err.println("** AbstractApplicationContext#.refresh()--销毁已经创建的单例以避免资源悬空。Destroy already created singletons to avoid dangling resources. ");
				destroyBeans();

	            System.err.println("** AbstractApplicationContext#.refresh()--重置“活动”标志。Reset 'active' flag. ");
				cancelRefresh(ex);

	            System.err.println("** AbstractApplicationContext#.refresh()--传播异常给调用者。Propagate exception to caller. ");
				throw ex;
			}
		}
	}
	 ...
	 * 
	 * 准备上下文刷新，设置其启动日期和活动标志以及执行属性源的任何初始化。
	 */
	protected void prepareRefresh() {
		...
		System.err.println("** AbstractApplicationContext#.refresh().prepareRefresh()--在上下文环境中初始化任何占位符属性源--For subclasses: do nothing by default.");
		initPropertySources();

		// Validate that all properties marked as required are resolvable 验证所有标记为必需的属性是否可解析
		// see ConfigurablePropertyResolver#setRequiredProperties
        System.err.println("** AbstractApplicationContext#.refresh().prepareRefresh()--验证所有标记为必需的属性是否可解析");
